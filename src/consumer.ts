...
async function handleMint(blockInscriptions: Inscription[]) {
  ...
  for (const inscription of blockInscriptions) {
    if (
      inscription.trimContent !==
      `{"p":"dot-20","op":"mint","tick":"${inscription.content.tick}"}`
    ) {
      console.warn(
        `Mint "${inscription.content.tick}" content is invalid`,
        JSON.stringify(inscription),
      );
      continue;
    }
    ...
    tickUserInscriptions[inscription.content.tick][inscription.to] =
      inscription;
  }
  ...
  for (const [tick, userInscriptions] of Object.entries(tickUserInscriptions)) {
    ...
    const existUserAccounts = await prisma.account.findMany({
      where: {
        address: {
          in: allInscriptions.map((i) => i.to),
        },
        tick,
      },
    });
    const newUserAccounts = allInscriptions.filter(
      (i) => !existUserAccounts.some((a) => a.address === i.to),
    );
...
async function handleTransfer(inscriptions: Inscription[]) {
  for (const inscription of inscriptions) {
    if (
      inscription.trimContent !==
      `{"p":"dot-20","op":"transfer","tick":"${inscription.content.tick}"}`
    ) {
      console.warn(
        `Transfer "${inscription.content.tick}" content is invalid`,
        JSON.stringify(inscription),
      );
      continue;
    }
